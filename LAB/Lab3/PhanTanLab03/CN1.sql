...
CN1 phân quyền cho các tk cn2
...
FROM cn1.PACKSERVICE p WHERE p.PackID NOT IN (SELECT PackID FROM cn1.STATIONSERVICE_STAFF WHERE StationID = 'Station1') AND EXISTS (SELECT 1 FROM cn2.STATIONSERVICE_STAFF@cn2_dblink_staff1 WHERE StationID = 'Station2');

Query 2: 
SELECT s1.PackId, p1.PromoID, p1.Rate
FROM cn1.STATIONSERVICE_ADMIN s1
JOIN cn1.PROMOTION p1 ON s1.PackId = p1.PackId AND p1.EndDate > TO_DATE('18/11/2023', 'DD/MM/YYYY')
...
JOIN cn2.PROMOTION@cn2_dblink_ga p2 ON s2.PackId = p2.PackId AND p2.EndDate > TO_DATE('18/11/2023', 'DD/MM/YYYY')
WHERE (s2.JamRate > 0.5 OR s2.NumberDom > 1000);

Query 3:
SELECT p1.PackId, COUNT(p1.PromoID) AS PromoCount1
FROM cn1.PROMOTION p1 
WHERE EXTRACT(MONTH FROM p1.StartDate) = 11 AND EXTRACT(MONTH FROM p1.EndDate) = 11
GROUP BY p1.PackId
UNION 
SELECT p2.PackId, COUNT(p2.PromoID) AS PromoCount2
FROM cn2.PROMOTION@cn2_dblink_admin1 p2
WHERE EXTRACT(MONTH FROM p2.StartDate) = 11 AND EXTRACT(MONTH FROM p2.EndDate) = 11
GROUP BY p2.PackId;
...
UNION
SELECT s2.PackId, 
       SUM(p2.PricePerMonth * s2.NumberDom * (1 - 0.125)) AS TotalRevenue
FROM cn2.STATIONSERVICE_ADMIN@cn2_dblink_ga s2
...
WHERE st1.StationID IN (
    ...
    FROM (
        ...
        FROM cn1.STATIONSERVICE_ADMIN s1
        WHERE s1.PackId IN (
            SELECT PackID FROM cn1.PACKSERVICE p1 WHERE p1.SpeedDown > 200
        )
...
AND st1.StationID NOT IN (
    SELECT StationID
    FROM cn1.PROMOTION p1
    WHERE p1.Rate < 0.5 AND p1.EndDate > TO_DATE('18/11/2023', 'DD/MM/YYYY')
)
UNION
...
WHERE st2.StationID IN (
    ...
    FROM (
        ...
        FROM cn2.STATIONSERVICE_ADMIN@cn2_dblink_ga s2
        WHERE s2.PackId IN (
            SELECT p2.PackID FROM cn2.PACKSERVICE@cn2_dblink_ga p2 WHERE p2.SpeedDown > 200
        )
...
AND st2.StationID NOT IN (
    ...
    WHERE p2.Rate < 0.5 AND p2.EndDate > TO_DATE('18/11/2023', 'DD/MM/YYYY')
);
